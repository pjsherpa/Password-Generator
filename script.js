...
function generatePassword(){
  // passwordText is what I will need to find. I kept this in the local scope and not the global scope as it will just add into the password previously created.-PJ
  var passwordText='';
  ...
  var x=prompt("Would you like symbols in your password y for Yes or n for No?")
  if(x==="y"){
  x = specialCharacter;
  }else if(x==="n"){
  x=""
  }else{
  window.alert("please select y for Yes or n for No")
  return;
  }
 
// last set of question for password criteria is to give a value to y, which will be our length-PJ
  var y=prompt("What would you like your password length to be?(Please select from minimum-8 to maximum-128)")
  if(y>= 8 && y <= 128){
  y;
  let length=Number(y);
  ...
  alert("Please make sure the length is between 8-128 characters long")
  return;
  }
  
  /*for the final step I have concatinated v,w,x into one variable named characters. 
  I have assigned the length which I got from var y on the loop and then assigned both the character and lenght to math.random function to generate the password - PJ*/
